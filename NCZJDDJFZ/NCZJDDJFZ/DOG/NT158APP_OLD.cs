...
namespace Dog.NT158.OLD
{
    public class NT158App
    {
        ...
        {
            key.UserPin1 = 1477462876;
            key.UserPin2 = -223285517;
            key.UserPin3 = -996008857;
            key.UserPin4 = 1481388299;
        ...
        public bool Authenticated()
        {
            ...
            if (checkDog() != 0)
            {
                MessageBox.Show("没有找到加密狗!", "不动产调查基础应用软件", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                Iscg = false;
                return false;
            }
            ...
            if (Login_Click() != 0)
            {
                MessageBox.Show("加密登录失败!", "不动产调查基础应用软件", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                Iscg = false;
                return false;
            }
            if (!ReadData())
        ...
        private bool ReadData()
        {
            ...
            DateTime DogData = Convert.ToDateTime(LR10).Date;//狗里存储的日期
            DateTime dqrq = Convert.ToDateTime(LR21).Date;//狗里存储的限制日期
            dqrq = dqrq.AddYears(-7);//将狗里的存储限制日期减去7年
            DateTime now = DateTime.Now.Date;//当前日期
            int gqbj = DogData.CompareTo(now);//当前日期和里存储的日期比较
            if (gqbj > 0)
            {
                MessageBox.Show("对不起，您不能修改系统日期!", "管理系统提示", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                return false;
            }
            int cr = now.CompareTo(dqrq);//当前日期和狗里存储的限制日期比较
            if (cr > 0)
            {
            ...
            else
            {
                ...
                //Rtn = NT158App.NT158WriteFile(keyHandles[0], 0, 0, pBuffer);
                ...
                else
                {
                    ...
                    // Rtn = NT158App.NT158WriteFile(keyHandles[0], 0, 0, wBuffer);
